...
public class BukkitFile implements YamlFile {
    private final JavaPlugin plugin;
    private final String pathToYamlFile;
    private final File rawFile;
    private FileConfiguration yamlFile;

    protected BukkitFile(JavaPlugin plugin, String pathToYamlFolder, String pathToYamlFile) {
        this.plugin = plugin;
    ...
    public void set(String path, Object object) {
        switch (object) {
            case Inventory inventory -> handleInventorySet(path, inventory);
            case Entity entity -> handleEntitySet(path, entity);
            case BlockData blockData -> yamlFile.set(path + ".blockData", blockData.getAsString());
            case Component[] components -> handleComponentSet(path, components);
            case UUID uuid -> yamlFile.set(path, uuid.toString());
            case null, default -> yamlFile.set(path, object);
        }
    }

    private void handleInventorySet(String path, Inventory inventory) {
    ...
    private void handleEntitySet(String path, Entity entity) {
        ...
        if (entity instanceof LivingEntity living) {
            handleLivingEntitySet(path, living);
        }

        handlePersistentData(path, entity);
    }

    private void handleLivingEntitySet(String path, LivingEntity living) {
    ...
    private void handlePersistentData(String path, Entity entity) {
        PersistentDataContainer container = ((PersistentDataHolder) entity).getPersistentDataContainer();
        for (NamespacedKey key : container.getKeys()) {
            Object data = container.get(key, PersistentDataType.STRING);
            if (data != null) {
    ...
    private void handleComponentSet(String path, Component[] components) {
        for (int i = 0; i < components.length; i++) {
            Component component = components[i];
            if (component instanceof TextComponent text) {
                handleTextComponentSet(path, i, text);
            }
        }
    }

    private void handleTextComponentSet(String path, int index, TextComponent text) {
        yamlFile.set(path + ".content." + index + ".text", text.content());

        if (text.hoverEvent() != null) {
            HoverEvent<?> hover = text.hoverEvent();
            yamlFile.set(path + ".content." + index + ".hoverAction", hover.action().toString());
            yamlFile.set(path + ".content." + index + ".hoverValue", ((Component) hover.value()).toString());
        }

        if (text.clickEvent() != null) {
            ClickEvent click = text.clickEvent();
            yamlFile.set(path + ".content." + index + ".clickAction", click.action().name());
            yamlFile.set(path + ".content." + index + ".clickValue", click.value());
        }
    }
    ...
    public Entity getEntity(String path, Entity defaultEntity) {
        ...
        return world.spawnEntity(loc, type, CreatureSpawnEvent.SpawnReason.CUSTOM, entity -> {
            configureEntityFromYaml(path, entity);
        });
    }
    ...
    private Location getLocationFromConfig(String path) {
        return new Location(
                Bukkit.getWorld(yamlFile.getString(path + ".location.world")),
                yamlFile.getDouble(path + ".location.x"),
                yamlFile.getDouble(path + ".location.y"),
                yamlFile.getDouble(path + ".location.z"),
                (float) yamlFile.getDouble(path + ".location.yaw"),
                (float) yamlFile.getDouble(path + ".location.pitch")
        );
    }

    private void configureEntityFromYaml(String path, Entity entity) {
        ...
        entity.setVelocity(new Vector(
                yamlFile.getDouble(path + ".velocity.x", 0),
                yamlFile.getDouble(path + ".velocity.y", 0),
                yamlFile.getDouble(path + ".velocity.z", 0)
        ));
        ...
        if (entity instanceof LivingEntity living) {
            configureLivingEntityFromYaml(path, living);
        }

        configurePersistentDataFromYaml(path, entity);
    }

    private void configureLivingEntityFromYaml(String path, LivingEntity living) {
        ...
        if (yamlFile.contains(path + ".potionEffects")) {
            handlePotionEffects(path, living);
        }
    }

    private void handlePotionEffects(String path, LivingEntity living) {
    ...
    private void configurePersistentDataFromYaml(String path, Entity entity) {
